...
1. Edit post screen (for earlier versions of WordPress or using the 'gi_wp_subtitles_use_meta_box' filter)
...
* Allow subtitle post meta key to be filtered using `gi_wp_subtitles_key`.
...
= 2.2 =
* Added 'gi_wp_subtitles_use_meta_box' filter to allow the edit field to be displayed in a meta box (the old way).
...
= 2.0 =
* Added custom post type support - use add_post_type_support( '{post_type}', 'gi_wp_subtitles' ).
* Added 'wps_meta_box_title' filter.
* Added 'gi_wp_subtitles' filter.
* Added 'gi_wp_subtitles_field_description' filter.
